    ...
    <div class="d-flex flex-row justify-content-between">
        <div class="container d-flex">
            <div class="welcome d-flex">
                <div class="name">
                    <h1 class="text-hblack p-0 m-0"><strong class="p-0">Add License,</strong></h1>
    ...
    <div class="section section-divider bills-of-license-purchased reveal mt-0">
        <div class="d-flex flex-column">
            <div class="section d-flex flex-column align-items-center mb-0">
                ...
                <h2 class="text-hblack font-weight-bold text-capitalize mt-3 sub-heading">Allocate New License</h2>
            </div>
            <div class="d-flex flex-column justify-content-around mb-3 mt-3 ml-5 section-divider my-card my-card-border">
                <form action="{{ route('licenses.store') }}" method="POST" id="create-license-form" enctype="multipart/form-data" class="border border-light shadow-lg p-4">
                    @csrf
                                ...
                                <select name="software_id" id="Software" class="form-control d-inline select2">
                                    ...
                                    @foreach ($softwares as $software)
                                        <option value="{{$software->id}}"
                                            {{ (old('software') ? 'selected' : '')}}>{{$software->software_name}}
                                        </option>
                                    @endforeach
                                </select>
                                @error('software')
                                ...
                                <select name="buyer_id" id="Buyer" class="form-control select2" onChange="setUser()">
                                    ...
                                    @foreach ($buyers as $user)
                                        <option value="{{$user->id}}"
                                            {{ (old('buyer_id') ? 'selected' : '')}}>{{$user->name}}
                                        </option>
                                    @endforeach
                                </select>
                                @error('buyer_id')
                        ...
                        <div class="d-flex flex-row justify-content-center">
                            <div class="form-group col-md-10">
                                ...
                                <textarea
                                    ...
                                    id="notes"
                                    rows="4"
                                    class="form-control  @error('notes') is-invalid @enderror" >{{old('description')}}</textarea>
                                @error('notes')
                                    <div class="text-danger">{{ $message }}</div>
                                @enderror
                            </div>
